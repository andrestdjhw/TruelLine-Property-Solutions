...
// DEV NOTE: Replace with real hardscape/concrete work photo
$img_hero = $theme_uri . '/assets/images/hardscape-hero.jpg';
?>
...
<style>
  ...
  /* ══ PAGE HEADER ══ */
  .sv-header { background: linear-gradient(160deg, #091914 0%, #0f2015 60%, #133429 100%); padding: 80px 32px 88px; text-align: center; position: relative; overflow: hidden; }
  ...
  .sv-hero-label { display: inline-flex; align-items: center; gap: 7px; background: rgba(42,90,73,0.1); border: 1px solid rgba(42,90,73,0.25); color: rgba(42,160,90,0.9); font-size: 10.5px; font-weight: 700; letter-spacing: 0.18em; text-transform: uppercase; padding: 5px 14px; border-radius: 3px; margin-bottom: 24px; }
  ...
  .sv-service-item { background: #fff; border-radius: 6px; padding: 28px 26px; border-left: 3px solid var(--accent); box-shadow: 0 2px 14px rgba(9,25,20,0.06); transition: transform 0.25s, box-shadow 0.25s, background 0.2s; display: flex; gap: 18px; align-items: flex-start; }
  .sv-service-item:hover { transform: translateY(-4px); box-shadow: 0 10px 30px rgba(9,25,20,0.11); background: #FCF7EC; }
  .sv-service-icon { width: 42px; height: 42px; background: var(--accent-soft); border: 1px solid var(--accent-border); border-radius: 6px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  ...
  .sv-areas-inner { max-width: 1200px; margin: 0 auto; display: flex; align-items: center; gap: 24px; flex-wrap: wrap; }
  ...
  .sv-area-item { font-family: 'Barlow Condensed', sans-serif; font-size: 14px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: var(--muted); text-decoration: none; padding: 4px 14px; border-right: 1px solid rgba(9,25,20,0.1); transition: color 0.2s; }
...
(function () {
  var observer = new IntersectionObserver(function (entries) {
    entries.forEach(function (e) {
      if (e.isIntersecting) { e.target.classList.add('sv-in'); observer.unobserve(e.target); }
    });
